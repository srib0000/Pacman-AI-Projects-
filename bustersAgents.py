...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        # Updating beliefs for all ghosts using both observe and elapseTime methods
        for index, infModule in enumerate(self.inferenceModules):
            # Only update if ghost is not yet captured
            if gameState.getLivingGhosts()[index + 1]:
                infModule.elapseTime(gameState)
                infModule.observeUpdate(gameState.getNoisyGhostDistances()[index], gameState)

        # Now finding the most likely position for each ghost
        mostLikelyGhostPositions = [inf.getBeliefDistribution().argMax() for inf in self.inferenceModules]

        # Choosing the action that brings Pacman closest to the nearest ghost
